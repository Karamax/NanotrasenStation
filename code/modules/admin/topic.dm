...
//				message_admins("[key_name_admin(usr)] has triggered a battle to the death (only one)")
			if("nofreestool")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","NFS")
				for(var/obj/structure/stool/W in world)
					new /mob/living/carbon/monkey/ignorant (W.loc)
				message_admins("[key_name_admin(usr)] activated monkeys everywhere mode")
		if(usr)
